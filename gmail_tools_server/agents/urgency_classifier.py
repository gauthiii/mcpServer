...
async def urgency_classifier_groq(answer,tool_mapping, tool_defs, model: str = "groq:llama-3.1-8b-instant") -> str:
    """
    ...
    prompt = f"""
    So have the necessay email subject and date information:

    {answer}

    Your task is it to extract the details of this email.
    Summarize this email information for the user.

    Classify them by urgency level: High, Medium, Low.
...
async def urgency_classifier_ollama(
    answer,
    tool_mapping,
    ...
    planning_prompt = f"""
    So you have the necessay email subject and date information:
...
async def urgency_classifier_openai(answer,tool_mapping, tool_defs, model: str = "openai:gpt-4o-mini") -> str: 
