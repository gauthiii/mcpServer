...
async def reflector_groq(strategy, answer, tool_mapping, tool_defs, model: str = "groq:llama-3.1-8b-instant") -> str:
    """
    ...
    prompt = f"""
    So for the following stratgey planned for the user:

    {strategy}

    This was found: {answer}

    You need to make sure if there's any other alternate based on the
    ...
    Do not fetch more than 2 items to avoid excessive data.
...
async def reflector_ollama(
    ...
    planning_prompt = f"""
You are a reflective agent. The high-level strategy to follow was:
